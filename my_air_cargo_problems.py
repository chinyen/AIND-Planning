...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        for action in self.actions_list:
            ...
            for precond in action.precond_pos:
                if precond not in current_state.pos:
                    action_possible = False
                    break
            if action_possible:
                for precond in action.precond_neg:
                    if precond not in current_state.neg:
                        action_possible = False
                        break
    ...
    def result(self, state: str, action: Action):
        ...
        for effect in action.effect_add:
            if effect not in new_state.pos:
                new_state.pos.append(effect)
        ...
        for effect in action.effect_rem:
            if effect not in new_state.neg:
                new_state.neg.append(effect)
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        for goal_state  in self.goal:
            if goal_state not in current_state.pos:
                count += 1
        return count
